...
class User:
    ...
    @classmethod
    def save(cls, data ):
        query = "INSERT INTO users ( first_name , last_name , age, country, email , password , created_at, updated_at ) VALUES ( %(first_name)s , %(last_name)s , %(age)s, %(country)s , %(email)s, %(password)s, NOW() , NOW() );"
    ...
    def is_valid(user):
        ...
        if len(user['last_name']) < 3:
            ...
            is_valid = False
        if int(user["age"]) < 15 or user["age"] == None:
            flash("Must be at least 15 years old.")
            is_valid = False
